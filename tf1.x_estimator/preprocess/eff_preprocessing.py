...
def _center_crop(image, image_size):
    ...
    crop_window = tf.stack([offset_height, offset_width,
                            padded_center_crop_size, padded_center_crop_size])
    image = tf.image.cropped_image(image, crop_window, channels=3)
    image = tf.image.resize_bicubic([image], [image_size, image_size])[0]
    return image
...
def preprocess_for_train(image, use_bfloat16, image_height=IMAGE_SIZE,
    ...
    if augment_name:
        try:
            from preprocess import autoaugment  # pylint: disable=g-import-not-at-top
...
def preprocess_image(image, label,
    ...
    if is_training:
        ...
        image = preprocess_for_eval(
            image, use_bfloat16, image_height, image_width)
