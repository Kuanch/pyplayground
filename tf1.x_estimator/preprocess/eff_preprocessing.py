...
def _random_crop(image, image_size):
  ...
  image = tf.image.resize_bicubic([image],  # pylint: disable=g-long-lambda
                                  [image_size, image_size])[0]
...
def preprocess_for_train(image, use_bfloat16, image_height=IMAGE_SIZE,
                         image_width=IMAGE_SIZE, augment_name=None,
  ...
  """
  # image = _decode_and_random_crop(image_bytes, image_height)
  image = _flip(image)
...
def preprocess_for_eval(image, use_bfloat16, image_height=IMAGE_SIZE, image_width=IMAGE_SIZE):
  """Preprocesses the given image for evaluation.
  ...
  Returns:
    A preprocessed image `Tensor`.
  """
  # image = _decode_and_center_crop(image_bytes, image_height)
  image = tf.reshape(image, [image_height, image_width, 3])
  image = tf.image.convert_image_dtype(
      image, dtype=tf.bfloat16 if use_bfloat16 else tf.float32)
  return image
...
def preprocess_image(image, label,
  ...
  if is_training:
    image = preprocess_for_train(
        image, use_bfloat16, image_height, image_width, augment_name,
        randaug_num_layers, randaug_magnitude)
  else:
    image = preprocess_for_eval(image, use_bfloat16, image_height, image_width)
